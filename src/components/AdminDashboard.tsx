...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [studentTypeFilter, setStudentTypeFilter] = useState<'all' | 'hosteller' | 'dayscholar'>('all');
  const [activeTab, setActiveTab] = useState<'all' | 'active' | 'important' | 'resolved' | 'spam'>('active');
  ...
  const isDayScholar = (type?: string) =>
    type ? type.toLowerCase().replace(/[^a-z]/g, '').includes('dayscholar') : false;

  const isHosteller = (type?: string) =>
  ...
  const displayedComplaints = uniqueGrievances
    .filter((g) => {
      ...
      if (studentTypeFilter === 'hosteller' && !isHosteller(g.studentType)) {
        return false;
      }
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 to-slate-800 text-white p-6 rounded-2xl shadow-sm border border-slate-700">
        ...
        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
          <button
            ...
            className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-center border border-white/10 transition-all cursor-pointer"
          >
          ...
          <button
            id="kpi-dayscholars"
            onClick={() => {
              setStudentTypeFilter('dayscholar');
              setSelectedCategory('All');
            }}
            className={`px-4 py-2 rounded-xl text-center border transition-all cursor-pointer ${
              studentTypeFilter === 'dayscholar'
                ? 'bg-purple-600 border-purple-400 text-white shadow-md'
                : 'bg-purple-500/20 hover:bg-purple-500/30 text-purple-200 border-purple-500/30'
            }`}
          >
            <div className="text-xl font-bold font-mono text-white">{dayscholarCount}</div>
            <div className="text-[10px] uppercase tracking-wider">Dayscholars</div>
          </button>
          <button
            id="kpi-hostellers"
            onClick={() => {
              setStudentTypeFilter('hosteller');
              setSelectedCategory('All');
            }}
            className={`px-4 py-2 rounded-xl text-center border transition-all cursor-pointer ${
              studentTypeFilter === 'hosteller'
                ? 'bg-blue-600 border-blue-400 text-white shadow-md'
                : 'bg-blue-500/20 hover:bg-blue-500/30 text-blue-200 border-blue-500/30'
            }`}
          >
            <div className="text-xl font-bold font-mono text-white">{hostellerCount}</div>
            <div className="text-[10px] uppercase tracking-wider">Hostellers</div>
          </button>
          <button
            id="kpi-urgent"
            onClick={() => {
            ...
            className="bg-red-500/20 hover:bg-red-500/30 text-red-200 px-4 py-2 rounded-xl text-center border border-red-500/30 transition-all cursor-pointer"
          >
            <div className="text-xl font-bold font-mono text-white">{importantComplaints.length}</div>
            <div className="text-[10px] uppercase tracking-wider text-red-200">Urgent</div>
          </button>
          <button
            id="kpi-resolved"
            onClick={() => {
            ...
            className="bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-200 px-4 py-2 rounded-xl text-center border border-emerald-500/30 transition-all cursor-pointer"
          >
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
          {/* View Mode Tabs */}
          <div className="flex gap-1.5 p-1 bg-slate-100 rounded-xl w-full sm:w-auto flex-wrap sm:flex-nowrap">
            <button
              id="admin-tab-all"
            ...
            >
              All Complaints ({grievances.length})
            </button>
            <button
              id="admin-tab-active"
          ...
          {/* Student Residency Sub-Filter & Search */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-1.5 p-1 bg-slate-100 rounded-xl">
              <span className="text-[10px] font-bold uppercase text-slate-400 px-2">Filter Residency:</span>
              <button
                id="residency-filter-all"
                onClick={() => {
                  setStudentTypeFilter('all');
                  setSelectedCategory('All');
                }}
                className={`px-2.5 py-1 text-[11px] font-semibold rounded-lg transition-all ${
                  studentTypeFilter === 'all' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                All Students
              </button>
              <button
                id="residency-filter-dayscholar"
                onClick={() => {
                  setStudentTypeFilter('dayscholar');
                  setSelectedCategory('All');
                }}
                className={`px-2.5 py-1 text-[11px] font-semibold rounded-lg transition-all flex items-center gap-1 ${
                  studentTypeFilter === 'dayscholar' ? 'bg-purple-600 text-white shadow-xs' : 'text-slate-600 hover:text-purple-700'
                }`}
              >
                Dayscholars ({dayscholarCount})
              </button>
              <button
                id="residency-filter-hosteller"
                onClick={() => {
                  setStudentTypeFilter('hosteller');
                  setSelectedCategory('All');
                }}
                className={`px-2.5 py-1 text-[11px] font-semibold rounded-lg transition-all ${
                  studentTypeFilter === 'hosteller' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-600 hover:text-blue-700'
                }`}
              >
